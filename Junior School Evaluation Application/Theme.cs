using System;
using System.Collections.Generic;
using System.Drawing;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace Junior_School_Evaluation_Application
{
    public class Theme
    {
        public static Color primary = Color.FromArgb(105, 65, 198);
        public static Color secondary = Color.FromArgb(125, 116, 99);
        public static Color success = Color.FromArgb(84, 180, 53);
        public static Color danger = Color.FromArgb(207, 10, 10);
        public static Color info = Color.FromArgb(25, 167, 206);
        public static Color warning = Color.FromArgb(248, 222, 34);
    }
}
